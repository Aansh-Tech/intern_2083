...
// // );
// //   // Extract primary image URL from images[] (polymorphic backend response)
// //   let imageUrl: string | null = null;
// //   if (item.images && item.images.length > 0) {
// //     const primary = item.images.find((img) => img.is_primary) ?? item.images[0];
// //     const imgSrc = primary?.image?.url ?? primary?.image?.image_path;
// //     if (imgSrc) imageUrl = resolveImageUrl(imgSrc);
// //     console.log("[certificateService] mapCertificate extracted image from images[]:", imageUrl?.substring(0, 80));
// //   } else if (item.image) {
...
//     console.log("[certificateService] imgSrc:", imgSrc);

//     if (imgSrc) {
//       imageUrl = resolveImageUrl(imgSrc);
//     }

//     console.log("[certificateService] imageUrl:", imageUrl);
//   }
...
// export async function getCertificates() {
//   const response = await api.get("/v1/certificates");
//   const items: BackendCertificate[] = response.data.data ?? response.data ?? [];
//   return items.map(mapCertificate);
// }

// export async function getCertificate(id: string) {
//   const response = await api.get(`/v1/certificates/${id}`);
//   const item: BackendCertificate = response.data.data ?? response.data;
//   return mapCertificate(item);
// }
