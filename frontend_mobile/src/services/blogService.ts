...
export const getBlog = async (slug: string) => {
  const response = await api.get(`/v1/admin/blog-posts/${slug}`);
  return unwrapItem(response);
};

export const createBlog = async (data: any) => {
  const response = await api.post("/v1/admin/blog-posts", data);
  return unwrapItem(response);
};

export const updateBlog = async (id: number | string, data: any) => {
  const response = await api.put(`/v1/admin/blog-posts/${id}`, data);
  return unwrapItem(response);
};

export const deleteBlog = async (id: number | string) => {
  const response = await api.delete(`/v1/admin/blog-posts/${id}`);
  return response.data;
};
